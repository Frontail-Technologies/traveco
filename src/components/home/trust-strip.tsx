...
export function TrustStrip() {
  return (
    <section className="bg-white py-10 md:py-14 border-b border-navy/10 relative overflow-hidden">
      <div className="mx-auto max-w-340 px-5 sm:px-6 lg:px-8">
        ...
        >
            ...
            {KEY_STATS.map((stat, idx) => {
              ...
              return (
                <motion.div
                  key={idx}
                  variants={fadeUp}
                  className="flex items-center gap-4.5 rounded-[1.5rem] border border-navy/10 bg-navy/[0.025] p-5 sm:p-6 transition-all duration-300 hover:border-accent/40 hover:bg-white hover:shadow-xs"
                >
                  <div className="flex size-12 shrink-0 items-center justify-center rounded-2xl bg-accent/10 text-accent">
                    <Icon className="size-5.5" />
                  </div>
                  <div>
                    <span className="text-3xl font-extrabold text-navy tracking-tight block leading-none mb-1">
                      {stat.value}
                    </span>
          ...
          <motion.div
            variants={fadeUp}
            className="rounded-[1.75rem] border border-navy/10 bg-muted/20 p-5 sm:p-7 shadow-2xs"
          >
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-5 pb-4 border-b border-navy/10">
              <div className="flex items-center gap-2">
                <ShieldCheck className="size-5 text-accent shrink-0" />
                <h3 className="text-lg sm:text-xl font-bold text-navy tracking-tight">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3">
              {TRUST_POINTS.map((point, idx) => (
                <div
                  key={idx}
                  className="flex items-start gap-2.5 rounded-xl border border-navy/10 bg-white p-3 shadow-2xs"
                >
