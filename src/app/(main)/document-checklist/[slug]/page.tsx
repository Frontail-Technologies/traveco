  ...
  return (
    <div className="min-h-screen bg-[#F7F5EF]">
      ...
      >
        ...
        <div className="relative z-20 mx-auto w-full max-w-340 px-4 sm:px-6 lg:px-8 flex flex-col items-start">
          {/* ── BACK ──────────────────────────────────────────── */}
          <Link
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center gap-6 md:gap-10 lg:gap-14">
            ...
            <div className="text-left">
              ...
              {/* ── METADATA STRIP ────────────────────────────────────────── */}
              <div className="flex flex-wrap items-center gap-6 border-y border-white/20 py-4">
                <div>
      ...
      <div className="mx-auto w-full max-w-340 px-4 sm:px-6 lg:px-8 pt-16 sm:pt-20">
          ...
          <div className="flex-1 min-w-0 max-w-215">
            ...
            <section id="documents" className="mb-14 scroll-mt-28">
              <div className="flex items-center gap-4 mb-6">
                <h2 className="text-[22px] font-semibold text-()">
                  <span className="mr-2 font-bold text-()">1.</span>
                  Documents Required
                </h2>
                <div className="h-px flex-1 bg-()/10" />
              </div>

              <p className="text-[15px] text-()/70 leading-relaxed mb-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {dest.coreChecklist.map((item, idx) => (
                  <div
                    key={idx}
                    className="flex items-start gap-3 bg-white border border-()/10 rounded-[10px] px-4 py-3 min-h-12"
                  >
                    <Check
                      className="w-4 h-4 shrink-0 mt-0.5 text-()"
                      strokeWidth={2.5}
                    />
                    <span className="text-[14px] font-medium leading-snug text-()">
                      {fmt(item)}
                    </span>
                  </div>
                ))}
            ...
            <section id="processing" className="mb-14 scroll-mt-28">
              <div className="flex items-center gap-4 mb-6">
                <h2 className="text-[22px] font-semibold text-()">
                  <span className="mr-2 font-bold text-()">2.</span>
                  Indicative Processing
                </h2>
                <div className="h-px flex-1 bg-()/10" />
              </div>

              <div className="bg-white border border-()/10 rounded-[10px] px-6 py-5">
                <p className="text-[15px] font-medium leading-relaxed text-()">
                  {dest.indicativeProcessing}
                </p>
              </div>
              <p className="text-[13px] text-()/60 mt-3 leading-relaxed">
            ...
            <section id="fees" className="mb-14 scroll-mt-28">
              <div className="flex items-center gap-4 mb-6">
                <h2 className="text-[22px] font-semibold text-()">
                  <span className="mr-2 font-bold text-()">3.</span>
                  Fees &amp; Charges
                </h2>
                <div className="h-px flex-1 bg-()/10" />
              </div>
              ...
              {liveFee ? (
                <div className="bg-white border border-()/10 rounded-[10px] px-5 py-4">
                  <p className="text-[10px] font-bold tracking-widest uppercase mb-2 text-()">
                    Live Fee Verification Required
                  </p>
                  <p className="text-[14.5px] font-medium leading-relaxed mb-2 text-()">
                    {feeText}
                  </p>
                  <p className="text-[13px] text-()/60 leading-relaxed">
            ...
            <section id="note" className="mb-14 scroll-mt-28">
              <div className="flex items-center gap-4 mb-6">
                <h2 className="text-[22px] font-semibold text-()">
                  <span className="mr-2 font-bold text-()">4.</span>
                  Before You Apply
                </h2>
                <div className="h-px flex-1 bg-()/10" />
              </div>

              <div className="bg-white border border-()/10 rounded-[12px] px-6 py-5">
                <p className="text-[10px] font-bold tracking-widest text-()/50 uppercase mb-3">Note</p>
                <p className="text-[14.5px] text-()/80 leading-relaxed">
      ...
      <PreFooterCTA countryName={dest.country} />
    </div>
  );
}
