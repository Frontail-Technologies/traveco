...
export default function VisaServicesHub() {
  return (
    <div className="bg-[#F7F5EF] pt-[88px]">
      ...
      <section className="py-20 md:py-28 lg:py-32 border-t border-[var(--traveco-navy)]/5">
        <div className="mx-auto max-w-340 px-5 sm:px-6 lg:px-8">
              ...
              return (
                <div key={service.id} className="flex flex-col group">
                  <div className="flex items-center gap-4 mb-6">
                    <span className="text-[var(--traveco-navy)]/30 font-medium text-[14px]">
                      0{index + 1}
                    </span>
                    <div className="w-10 h-[1px] bg-[var(--traveco-navy)]/10" />
                  </div>
                  
                  <div className="flex items-start gap-5">
                    <div className="w-12 h-12 rounded-[12px] bg-white border border-[var(--traveco-navy)]/10 flex items-center justify-center shrink-0 shadow-sm group-hover:border-[var(--traveco-gold)] transition-colors">
                      <Icon className="w-5 h-5 text-[var(--traveco-navy)]" />
                    </div>
                    <div>
                      <h3 className="text-[22px] md:text-[26px] font-medium text-[var(--traveco-navy)] mb-4">
                        {service.title}
                      </h3>
                      <p className="text-[var(--traveco-navy)]/70 text-[15px] leading-[1.65] mb-6">
                        {service.description}
                      </p>
                      <Link 
                        href={`/visa-services/${service.slug}`}
                        className="inline-flex items-center text-[14px] font-bold text-[var(--traveco-navy)] hover:text-[var(--traveco-gold)] transition-colors"
                      >
